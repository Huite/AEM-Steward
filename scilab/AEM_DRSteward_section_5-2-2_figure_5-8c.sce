...
title_author="AEM DRSteward Section 5.2.2, Figure 5.8c";
printf("  %s\n\n",title_author);
...
function [solve_type]=solve_type_Psi_uniform()
  solve_type="solution type: stream function uniform"; 
endfunction
...
//   solve_type==solve_type_Psi_uniform()    set to [] since unused
...
// M: number of control points where boundary condition is applied; use default if not specified
  ...
  if solve_type==solve_type_Psi_uniform() then
    ...
    for n=0:N
      ...
      line_ielt.solve_A=[line_ielt.solve_A,imag(%i*omega_double)]; 
    end
  end
...
function [change]=line_flow_solve()
  ...
  change=0;
  for ielt=1:max(size(line))
    omega_beforesolve=line_flow_omega(line(ielt).solve_z,[1:ielt-1,ielt+1:max(size(line))]);
    omega_add=omega_add_line_flow(line(ielt).solve_z,[ielt])
    if line(ielt).solve_type==solve_type_Psi_uniform() then
      line(ielt).solve_b=-imag(omega_add);
      coef=lsq(line(ielt).solve_A,line(ielt).solve_b);
      line(ielt).coef_double_layer=%i*coef(2:$);
...
for i=1:max(size(z1save))
  solve_type=solve_type_Psi_uniform(); solve_value=[];
