...
title_author="AEM DRSteward Section 3.4.2, Figure 3.24a";
printf("  %s\n\n",title_author);
...
function slit_flow_error()
  ...
  for ielt=1:size(slit)
    ...
    if slit(ielt).solve_type==solve_type_Psi_uniform() then
      slit(ielt).residual_error=imag(omega)-slit(ielt).coef_Psi;
    end
  end
  net_error=[];
  for ielt=1:size(slit)
