...
title_author="AEM DRSteward Section 3.4, Figure 3.19b";
printf("  %s\n\n",title_author);
...
Gamma=1;
omega=%i*Gamma/(2*%pi)*log( (((z+1).^(0.5)) .* ((z-1).^(0.5))) + z);
v    =%i*Gamma/(2*%pi)*( conj(  (zv+1).^(-0.5) .* (zv-1).^(-0.5) ));  

draw_flow_conduction(x,y,real(omega),[-0.5:.025:0.5]*Gamma,imag(omega),[-5:0.025:5]*Gamma,xv,yv,real(v),imag(v));
