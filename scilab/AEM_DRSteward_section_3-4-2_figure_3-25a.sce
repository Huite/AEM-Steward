...
title_author="AEM DRSteward Section 3.4.2, Figure 3.25a";
printf("  %s\n\n",title_author);
...
function [solve_type]=solve_type_Robin_dPhi()
  solve_type="solution type: Robin normal vector from discontinuous potential"; 
endfunction
...
//   solve_type==solve_type_Robin_dPhi() delta = regional k * slit width / slit k
...
// M: number of control points where boundary conditions are applied
  ...
  if sliti.solve_type==solve_type_Robin_dPhi() then
    sliti.solve_delta=solve_value;
  end
  ...
  if solve_type==solve_type_Robin_dPhi() then
    sliti.A=[]; for n=1:N sliti.A=[sliti.A,sin(n*sliti.solve_theta)]; end
    sliti.B=zeros(M,M);  xi=real(sliti.solve_zeta); for m=1:M  sliti.B(m,m)=1/(sliti.L*sqrt(xi(m)+1)*sqrt(1-xi(m))); end
    sliti.C=zeros(N,N);  for n=1:N  sliti.C(n,n)=n; end
    sliti.Asolve=sliti.B*sliti.A*sliti.C+2/sliti.solve_delta*sliti.A;
  end
...
function [change]=slit_flow_solve()
  ...
  for ielt=1:max(size(slit))
    ...
    if slit(ielt).solve_type==solve_type_Robin_dPhi() then
      v_solve_z=v_add_slit_flow(slit(ielt).solve_z,[ielt]);
      slit(ielt).coef=%i*(slit(ielt).Asolve\imag(-v_solve_z*slit(ielt).expminusialpha));     
    end
...
function slit_flow_error()
  ...
  for ielt=1:size(slit)
    ...
    if slit(ielt).solve_type==solve_type_Robin_dPhi() then
      vn=imag(v*slit(ielt).expminusialpha);
      delta_Phi=0; for n=1:max(size(slit(ielt).coef)) delta_Phi=delta_Phi+2*imag(slit(ielt).coef(n))*sin(n*slit(ielt).solve_theta); end
      slit(ielt).residual_error=vn+delta_Phi/slit(ielt).solve_delta;
    end
  end
  net_error=[];
  for ielt=1:size(slit)
...
solve_type=solve_type_Robin_dPhi(); solve_value=100;
