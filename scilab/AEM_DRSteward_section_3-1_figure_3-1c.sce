...
title_author="AEM DRSteward Section 3.1, Figure 3.1c";
printf("  %s\n\n",title_author);
...
v=v0*ones(zv);
Q=1; zp=.6-%i*.6;
omega=omega+Q/(2*%pi)*log(z-zp);
v    =v-Q/(2*%pi)*conj((zv-zp).^(-1));
Q=1; zp=.4+%i*.6;
omega=omega+Q/(2*%pi)*log(z-zp);
v    =v-Q/(2*%pi)*conj((zv-zp).^(-1));
Q=1; zp=-.4-%i*.4;
...
draw_flow_conduction(x,y,real(omega),[-1.5:.1:1.5],imag(omega),[-10:.1:10],xv,yv,real(v),imag(v));
