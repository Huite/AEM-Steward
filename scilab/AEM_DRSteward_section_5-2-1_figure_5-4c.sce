...
title_author="AEM DRSteward Section 5.2.1, Figure 5.4c";
printf("  %s\n\n",title_author);
...
// compute logarithm function for line element
global line; ielt=1; 
Z=(z-0.5*(line(ielt).z2+line(ielt).z1))./(0.5*(line(ielt).z2-line(ielt).z1));  R=abs(Z);  
omega=(log((Z+1)./(Z-1)))/(2*%pi);

draw_flow_conduction(x,y,imag(omega),[-0.5:1/20:0.5],[],[],[],[],[],[]);
