...
title_author="AEM DRSteward Section 3.4.3, Figure 3.27a";
printf("  %s\n\n",title_author);
...
// M: number of control points where boundary conditions are applied
  ...
  if sliti.solve_type==solve_type_Kutta() then
...
L=0.75; alpha=9.5/180*%pi; zwing=[-L;L]*exp(%i*alpha);
...
draw_flow_conduction(x,y,[],[],imag(omega),[-2:0.05:2],xv,yv,real(v),imag(v));
