...
title_author="AEM DRSteward Section 5.2.2, Figure 5.9c";
printf("  %s\n\n",title_author);
...
//   solve_type==solve_type_Robin_dPhi() delta = regional k * line width / line k
...
// M: number of control points where boundary condition is applied; use default if not specified
  ...
  if solve_type==solve_type_Robin_dPhi() then
    ...
    for n=0:N
      ...
      line_ielt.solve_A=[line_ielt.solve_A,imag(-%i*vsn_double)+(real(line_ielt.solve_Z)).^n/line_ielt.solve_delta]; 
    end
  end
...
function [change]=line_flow_solve()
  ...
  for ielt=1:max(size(line))
    ...
    if line(ielt).solve_type==solve_type_Robin_dPhi() then
      line(ielt).solve_b=-imag(v_add*conj(line(ielt).expitheta));
      coef=lsq(line(ielt).solve_A,line(ielt).solve_b);
      line(ielt).coef_double_layer=%i*coef;
    end
  ...
  for ielt=1:size(line)
    ...
    if line(ielt).solve_type==solve_type_Robin_dPhi() then
      ...
      for n=0:max(size(line(ielt).coef_double_layer))-1
        delta_Phi=delta_Phi+imag(line(ielt).coef_double_layer(n+1))*(real(Z)).^n;
      end
      line(ielt).residual_error=imag(v*conj(line(ielt).expitheta))+delta_Phi/line(ielt).solve_delta;
    end
  end
  net_error=[];
  for ielt=1:size(line)
...
for i=1:max(size(z1save))
  solve_type=solve_type_Robin_dPhi(); solve_value=1;
