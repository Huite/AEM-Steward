...
title_author="AEM DRSteward Section 3.1, Figure 3.4c";
printf("  %s\n\n",title_author);
...
omega=  -conj(v0)*z+v0*r0^2*(z-zc).^(-1)+Q/(2*%pi)*log(z-zc);
v    =v0+conj(v0)*r0^2*(conj(zv-zc)).^(-2)-Q/(2*%pi)*conj((zv-zc).^(-1));
...
draw_flow_conduction(x,y,real(omega),[-2:0.25:2]*abs(v0)-real(conj(v0)*zc),imag(omega),[-100:0.25:100]*abs(v0),xv,yv,real(v),imag(v));
