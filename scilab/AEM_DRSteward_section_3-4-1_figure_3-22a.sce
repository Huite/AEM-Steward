...
title_author="AEM DRSteward Section 3.4.1, Figure 3.22a";
printf("  %s\n\n",title_author);
...
//   solve_type==solve_type_Robin_dPsi() delta = slit k * slit width / regional k
...
// M: number of control points where boundary conditions are applied
  ...
  if sliti.solve_type==solve_type_Robin_dPsi() then
...
function [change]=slit_flow_solve()
  ...
  for ielt=1:max(size(slit))
    ...
    if slit(ielt).solve_type==solve_type_Robin_dPsi() then
      v_solve_z=v_add_slit_flow(slit(ielt).solve_z,[ielt]);
      slit(ielt).coef=slit(ielt).Asolve\real(v_solve_z*slit(ielt).expminusialpha);
    end
...
function slit_flow_error()
  ...
  for ielt=1:size(slit)
    ...
    if slit(ielt).solve_type==solve_type_Robin_dPsi() then
      vs=real(v*slit(ielt).expminusialpha);
      delta_Psi=0; for n=1:max(size(slit(ielt).coef)) delta_Psi=delta_Psi-2*slit(ielt).coef(n)*sin(n*slit(ielt).solve_theta); end
      slit(ielt).residual_error=vs+delta_Psi/slit(ielt).solve_delta;
    end
  end
  net_error=[];
  for ielt=1:size(slit)
...
solve_type=solve_type_Robin_dPsi(); solve_value=100;
